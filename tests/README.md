...
| `test_typed_api.cc` | **yes** | Assertions against concrete `ondewo::csi` / `ondewo::s2t` / `ondewo::t2s` C++ types |
...
CSI is a composition product, and the suite asserts the whole composed surface: all
**25** `.proto` files (its own `ondewo/csi/conversation.proto` plus the NLU, S2T and T2S trees
it imports) and all **19** services must be in the descriptor pool.
...
- `optional string language` of `ondewo::s2t::TranscribeRequestConfig` — a proto3 *explicit
  presence* field — still reports `has_language()` after an empty-string round trip, while the
...
- Service stubs are constructed against a channel, and a unary RPC (`GetS2sPipeline`), a
  **bidirectional** streaming RPC (`S2sStream`, a `ClientReaderWriter`) and a **server**-streaming
  ...
  request/response types and the generated method descriptors all link and dispatch.
...
**100 % of 312 lines and 73 of 73 functions**. The generated stubs are excluded from the
