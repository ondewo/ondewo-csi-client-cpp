...
`ondewo-csi-client-cpp` is the C++ gRPC client library for the
[ONDEWO CSI API](https://github.com/ondewo/ondewo-csi-api) - ONDEWO's Conversational Speech Interface service. It is a
...
| `ondewo-csi-api/`                   | submodule - the `.proto` source of truth                                 |
...
C++ has no package registry, so every [GitHub release](https://github.com/ondewo/ondewo-csi-client-cpp/releases)
carries the built package as an asset: `ondewo_csi_client-<version>-<platform>.tar.gz`, plus a
...
archive=ondewo_csi_client-${version}-${platform}.tar.gz

gh release download "${version}" --repo ondewo/ondewo-csi-client-cpp --pattern "${archive}*"
...
```cmake
find_package(ondewo_csi_client CONFIG REQUIRED)
target_link_libraries(my_app PRIVATE ondewo::ondewo_csi_client)
```

```shell
cmake -S . -B build -DCMAKE_PREFIX_PATH=/opt/ondewo_csi_client-${version}-${platform}
```
...
set(ONDEWO_LIBRARY_NAME ondewo_csi_client CACHE STRING "" FORCE)
FetchContent_Declare(
  ondewo_csi_client
  GIT_REPOSITORY https://github.com/ondewo/ondewo-csi-client-cpp.git
  GIT_TAG        5.5.0)
FetchContent_MakeAvailable(ondewo_csi_client)
...
target_link_libraries(my_app PRIVATE ondewo_csi_client)
```
...
```shell
git clone https://github.com/ondewo/ondewo-csi-client-cpp.git
cd ondewo-csi-client-cpp
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release \
  -DONDEWO_LIBRARY_NAME=ondewo_csi_client \
...
```shell
git clone https://github.com/ondewo/ondewo-csi-client-cpp.git   ## Clone repository
cd ondewo-csi-client-cpp                                        ## Change into repo directory
...
generated header you need (`api/ondewo/csi/<file>.grpc.pb.h`) to keep compile times down.
...
int main() {
  ...
  auto channel = grpc::CreateChannel("csi.ondewo.com:443", grpc::SslCredentials({}));

  // Replace Services/Request/Response with the service you need - see the API documentation.
  auto stub = ondewo::csi::Services::NewStub(channel);

  grpc::ClientContext context;
  context.AddMetadata("authorization", "Bearer " + std::string(getenv("ONDEWO_TOKEN")));

  ondewo::csi::Request request;
  ondewo::csi::Response response;
...
docker run \
  -v $(pwd):/input-volume \
  -v $(pwd):/output-volume \
  ondewo-cpp-proto-compiler ondewo-csi-api ondewo ondewo_csi_client
```
...
- In the output volume it deletes only what it owns - `api/`, `include/ondewo_csi_client/`,
  `lib/libondewo_csi_client.a` and `lib/cmake/ondewo_csi_client/` - so a proto that was renamed or
...
- `check_build` asserts that every `.proto` under `ondewo-csi-api/ondewo` produced a matching `.pb.h`.
- `smoke_test` writes a throwaway project that does `find_package(ondewo_csi_client CONFIG REQUIRED)`,
  includes `public-api.h` and links `ondewo::ondewo_csi_client`, then compiles and runs it. That is the
